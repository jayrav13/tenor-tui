...
class TestYahooProviderGetExpirations:
    def test_returns_expirations(self):
        ...
        provider = YahooProvider()
        with patch("tenortui.providers.yahoo.yf.Ticker", return_value=_mock_ticker({}, options=expirations)):
...
class TestYahooProviderGetChain:
    def test_returns_chain(self):
        ...
        provider = YahooProvider()
        with patch("tenortui.providers.yahoo.yf.Ticker", return_value=_mock_ticker({}, chain_data=chain_data)):
